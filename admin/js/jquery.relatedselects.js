...
(function($) {
	...
	var RelatedSelect = function(context, opts) {
		...
		$.each(opts.selects, function(elem, o) {
			var $select = $context.find('select[id="' + elem + '"]'), // jquery ref to this select box
		...
		function saveDefaultOptionText() {
			...
			for (var x = 1, len = selects.length; x < len; x++) {
				$select = $context.find('select[id="' + selects[x] + '"]');
		...
		function resetAfter(elem) {
			...
			for (var x = thispos + 1, len = selects.length; x < len; x++) {
				$('select[id="' + selects[x] + '"]', $context).attr('disabled', 'disabled').find('option:first').prop('selected', 'selected');
			}
		}

		function next(elem) {
			return $context.find('select[id="' + selects[getPosition(elem) + 1] + '"]');
		}
