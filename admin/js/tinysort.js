...
function ordenar(colNueva, obj) {
	var table = $(obj).parents('table')[0],
		...
		colVieja = $('#' + table.id + '-orden').val(),
		isAscending = $('#' + table.id + '-ordenTipo').val() === 'asc';

	if (colVieja !== '') {
		colVieja = parseInt(colVieja);
	}
	...
	if (colVieja !== '') {
		...
		if (colNueva == colVieja) {
			order = isAscending ? 'desc' : 'asc';
		}
	}

	$('#' + table.id + '-ordenTipo').val(order);
